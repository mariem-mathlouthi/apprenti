  ...
  <div class="flex flex-col h-screen bg-gray-100">
    ...
    <main class="flex-1 p-8 overflow-auto mt-16 ml-64">
      <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier votre avis</h1>
        
        <form @submit.prevent="updateFeedback" class="bg-white rounded-lg shadow-md p-6">
          <!-- Notation -->
          <div class="mb-6">
          ...
          <div class="flex justify-end space-x-4">
            ...
            <button
              ...
              class="px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition-all duration-200 disabled:opacity-50"
            >
    ...
    async fetchFeedback() {
      try {
        ...
        if (response.data.success) {
          this.form.note = response.data.feedback.note;
          this.form.commentaire = response.data.feedback.commentaire;
        }
      } catch (error) {
        console.error("Erreur lors du chargement du feedback:", error);
        toast.error("Impossible de charger le feedback à modifier");
        this.$router.push({ name: 'ConsultRessource', params: { id: this.idCours } });
      } finally {
