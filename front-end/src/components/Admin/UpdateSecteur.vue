<template>
          ...
          <div class="container mx-auto px-4 py-8">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
              <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                <div>
                  <h3 class="text-lg font-medium leading-6 text-gray-900">Modifier le secteur</h3>
                  <p class="mt-1 max-w-2xl text-sm text-gray-500">Mettez à jour ou supprimez le secteur</p>
                </div>
              </div>
  ...
  export default {
    ...
    created() {
      this.secteurId = this.$route.params.id;
      this.fetchSecteurDetails();
    },
    methods: {
      async fetchSecteurDetails() {
        try {
          this.loading = true;
          ...
          if (response.data?.success && response.data?.secteur) {
            this.description = response.data.secteur.description;
        ...
        try {
          ...
          if (response.status === 200) {
            toast.success("Secteur mis à jour avec succès !", { 
              autoClose: 2000,
              onClose: () => this.$router.push("/SecteurList")
            });
          } else {
      ...
      confirmDelete() {
        if (confirm("Êtes-vous sûr de vouloir supprimer définitivement ce secteur ?")) {
          this.deleteSecteur();
        }
      },
      ...
      async deleteSecteur() {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            throw new Error("Secteur introuvable ou déjà supprimé");
          }
        } catch (error) {
