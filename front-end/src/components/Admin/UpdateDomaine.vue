        ...
        <div class="container mx-auto px-4 py-8">
          <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
              <div>
                <h3 class="text-lg font-medium leading-6 text-gray-900">Modifier le domaine</h3>
                <p class="mt-1 max-w-2xl text-sm text-gray-500">Mettez à jour ou supprimez le domaine</p>
              </div>
            </div>
...
export default {
  ...
  created() {
    this.domaineId = this.$route.params.id;
  ...
  methods: {
    async fetchDomaineDetails() {
      try {
        ...
        if (response.data?.success && response.data?.domaine) {
          this.description = response.data.domaine.description;
      ...
      try {
        ...
        if (response.status === 200) {
          toast.success("Domaine mis à jour avec succès !", { 
            autoClose: 2000,
            onClose: () => this.$router.push("/DomaineList")
          });
        } else {
    ...
    confirmDelete() {
      if (confirm("Êtes-vous sûr de vouloir supprimer définitivement ce domaine ?")) {
        this.deleteDomaine();
      }
    },
    ...
    async deleteDomaine() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          throw new Error("Domaine introuvable ou déjà supprimé");
        }
      } catch (error) {
